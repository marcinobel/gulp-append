...
module.exports = function (fileOrStream, opt) {
    if (!fileOrStream) {
        throw new PluginError('gulp-append', 'Missing fileOrStream option for gulp-append');
    }
    if (!opt) {
        opt = {};
    }
    ...
    if (opt.transform && typeof opt.transform !== 'function') {
        throw new PluginError('gulp-append', 'transform option must be a function');
    }
    if (opt.sort && typeof opt.sort !== 'function') {
        throw new PluginError('gulp-append', 'sort option must be a function');
    }
    if (opt.adjust && typeof opt.adjust !== 'function') {
        throw new PluginError('gulp-append', 'adjust option must be a function');
    }
...
function handleVinylStream(toInject, opt) {
    ...
    return es.map(function (source, cb) {
        if (source.isStream()) {
            return cb(new PluginError('gulp-append', 'Streams not supported for source templates!'));
        }
